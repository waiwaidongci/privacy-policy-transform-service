...
func (s *PrivacyService) PutPolicy(ctx context.Context, p domain.TransformPolicy) (domain.TransformPolicy, error) {
	...
	p = domain.CloneTransformPolicy(p)
	s.mu.Lock()
	s.policies[p.ID] = p
	s.policyHistory[p.ID] = append(s.policyHistory[p.ID], domain.CloneTransformPolicy(p))
	s.mu.Unlock()
	return domain.CloneTransformPolicy(p), nil
}
...
func (s *PrivacyService) RollbackPolicy(id string) (domain.TransformPolicy, error) {
	...
	p := domain.CloneTransformPolicy(history[len(history)-2])
	p.Status = "published"
	p.UpdatedAt = time.Now().UTC()
	s.policies[id] = p
...
func (s *PrivacyService) ListResults() []domain.TransformResult {
	...
	for _, r := range s.results {
		out = append(out, domain.CloneTransformResult(r))
	}
	return out
}
