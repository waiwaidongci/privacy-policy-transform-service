...
func cloneTransformPolicy(p domain.TransformPolicy) domain.TransformPolicy {
	p.Rules = append([]domain.FieldRule(nil), p.Rules...)
	return p
}

func cloneTransformResult(r domain.TransformResult) domain.TransformResult {
	if r.Data != nil {
		r.Data = domain.DeepCopyJSON(r.Data)
	}
	return r
}
...
func (s *PrivacyService) PutPolicy(ctx context.Context, p domain.TransformPolicy) (domain.TransformPolicy, error) {
	...
	p = cloneTransformPolicy(p)
	s.mu.Lock()
	s.policies[p.ID] = p
	s.policyHistory[p.ID] = append(s.policyHistory[p.ID], cloneTransformPolicy(p))
	s.mu.Unlock()
	return cloneTransformPolicy(p), nil
}
...
func (s *PrivacyService) RollbackPolicy(id string) (domain.TransformPolicy, error) {
	...
	p := cloneTransformPolicy(history[len(history)-2])
	p.Status = "published"
	p.UpdatedAt = time.Now().UTC()
	s.policies[id] = p
...
func (s *PrivacyService) ListResults() []domain.TransformResult {
	...
	for _, r := range s.results {
		out = append(out, cloneTransformResult(r))
	}
	return out
}
func (s *PrivacyService) Transform(ctx context.Context, requestID, policyID string, input map[string]any) (domain.TransformResult, error) {
	...
	s.mu.Lock()
	s.results[requestID] = cloneTransformResult(res)
	s.mu.Unlock()
	return cloneTransformResult(res), nil
}
...
func (s *PrivacyService) GetResult(id string) (domain.TransformResult, error) {
	...
	return cloneTransformResult(r), nil
}
