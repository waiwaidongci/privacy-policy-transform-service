...
func (s *Service) PolicyPublication(ctx context.Context, ruleSetID string, revision int, envID, reason string) (domain.PolicyPublication, error) {
	...
	if !domain.CanTransitionRevision(v.Status, "published") {
		return domain.PolicyPublication{}, fmt.Errorf("%w: revision status %q cannot be published", domain.ErrConflict, v.Status)
	}
	now := s.now()
