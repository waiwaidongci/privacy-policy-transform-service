...
type Memory struct {
	...
	index        *TransformRuleSetIndex
}

func NewMemory() *Memory {
	return &Memory{workspaces: map[string]domain.PolicyWorkspace{}, envs: map[string]domain.ProcessingPurpose{}, ruleSets: map[string]domain.TransformRuleSet{}, revisions: map[string][]domain.TransformRevision{}, publications: map[string][]domain.PolicyPublication{}, index: NewTransformRuleSetIndex()}
}
...
func (m *Memory) CreateProcessingPurpose(_ context.Context, e domain.ProcessingPurpose) error {
	...
	e2 := e
	m.envs[e.ID] = cloneProcessingPurpose(e2)
	return nil
}
...
func (m *Memory) SaveTransformRevision(_ context.Context, v domain.TransformRevision) error {
	...
	for i, x := range arr {
		if x.Number == v.Number {
			arr[i] = cloneTransformRevision(v)
			m.revisions[v.TransformRuleSetID] = arr
			return nil
		}
	}
	m.revisions[v.TransformRuleSetID] = append(arr, cloneTransformRevision(v))
	return nil
}
...
func (m *Memory) ListTransformRevisions(_ context.Context, id string) ([]domain.TransformRevision, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	out := make([]domain.TransformRevision, 0, len(m.revisions[id]))
	for _, v := range m.revisions[id] {
		out = append(out, cloneTransformRevision(v))
	}
	return out, nil
}
